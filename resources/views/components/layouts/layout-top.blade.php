...
<body class="layout-3" style="font-family: 'Inter', sans-serif">
                ...
                <div class="container">
                    <ul class="navbar-nav">
                        ...
                        @if (Auth::user()->hasRole('admin'))
                            <li
                                class="nav-item dropdown {{ request()->is('short-film') || request()->is('control-user') ? 'active' : '' }}">
                                <a href="#" data-toggle="dropdown" class="nav-link has-dropdown">
                                    <i class="fas fa-chalkboard"></i><span>Master Data</span>
                                </a>
                                <ul class="dropdown-menu">
                                    <li class="nav-item {{ request()->is('short-film') ? 'active' : '' }}">
                                        <a href="/short-film" class="nav-link">Short Film</a>
                                    </li>
    ...
    <script>
        window.addEventListener('swal:modal', event => {
            ...
            $("#shotDataModal").modal("hide");
            $("#subShotDataModal").modal("hide");
        })
        window.addEventListener('swal:confirm', event => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Livewire.dispatch('deleteShot')
                    Livewire.dispatch('deleteSubShot')
                }
            })
        })
        window.addEventListener('swal:confirm:shot', event => {
            Swal.fire({
                title: event.detail[0].message,
                text: event.detail[0].text,
                icon: event.detail[0].type,
        ...
        window.addEventListener('swal:confirm:subshot', event => {
            Swal.fire({
                title: event.detail[0].message,
                text: event.detail[0].text,
                icon: event.detail[0].type,
